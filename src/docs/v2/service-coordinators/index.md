...
To summarize, any action expected to precede another should be abstracted as high as realistically required business wise, without callers concerning themselves with it. Candidates for centralization are validation, model hydration, conditionals, business logic. Actions preceding model operations should be hook into its [lifecycle methods](/docs/v2/authorization#Model-based-authorization). The solitary purpose of a service coordinator is to serve as conduit or juncture between these central points.

## Response Object Returns

In Suphle v2, service coordinator methods return response objects directly instead of arrays or primitive values. This provides better type safety, clearer intent, and more explicit control over the response format.
...
2. **Clear Intent**: The response format is obvious from the method signature
3. **IDE Support**: Better autocomplete and refactoring support
4. **Consistency**: All responses follow the same pattern
5. **Testability**: Easier to test specific response formats
...
class UserCoordinator extends BaseCoordinator
{
    ...
    {
        $user = $this->userService->create($this->payloadStorage->fullPayload());
...
The coordinator is automatically discovered by the router configuration:
...
class RouterMock extends Router
{
    ...
    {
        return []; // Empty array means scan all coordinators
    }
}
```

The most simplistic service coordinator, such as the one defined above, will return response objects directly. These response objects define the format and content of the HTTP response.
...
class ValidatorCoordinator extends BaseCoordinator {
	...
	public function postWithValidator(CartBuilder $cartBuilder): Json {

		$cart = $this->cartService->create($cartBuilder->getDomainObject());
		
		return new Json(['message' => 'Cart created successfully']);
	}
}
```
